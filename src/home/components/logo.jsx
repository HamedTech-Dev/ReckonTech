import { forwardRef } from "react";
import { Scroller } from "../../utility/scroller";
import logo from "../../assets/images/IMG_0578-removebg-preview.png";


const Logo = forwardRef((props, ref) => {
  return (
    <div
      className="w-[100vw] h-[100vh] bg-[var(--primary-light)] flex flex-col  items-center"
      ref={ref} id="home"
    >
      <div className=" w-full h-5/6 flex flex-col justify-center items-center logo-container">
        <img
          src={logo}
          alt="Logo"
          className="relative mobile:self-center mobile:top-[33.33333%] mobile:w-[150%] fade-in"
        />
      </div>
      <Scroller />
    </div>
  );
});

export default Logo;
